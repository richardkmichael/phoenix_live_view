defmodule Phoenix.LiveComponent do
  ...
  `Phoenix.LiveView.Helpers.live_redirect/2`. The [`live_patch/2`](`Phoenix.LiveView.Helpers.live_patch/2`)
  is always handled by the parent LiveView, as a component does not
  have a `c:Phoenix.LiveView.handle_params/3` callback.
  ...
  ### A component top-level may only contain HTML tags

  A component must only contain HTML tags at the top-level. At least one HTML
  tag must be present. It is not possible for a component to render
  only text or text mixed with tags at the top-level.
